...
export class WorkoutStore {
  ...
  startWorkout(name: string = `Workout ${new Date().toLocaleDateString()}`): Workout {
  ...
  startWorkoutFromTemplate(templateId: string): Workout | null {
    ...
    template.exercises.forEach(exercise => {
      const movement = this.rootStore.movementStore.getMovement(exercise.movementId);
      if (movement) {
        this.addExerciseToWorkout(newWorkout.id, exercise.movementId, exercise.sets);
      }
    });
  ...
  addExerciseToWorkout(
    ...
    const sets: WorkoutSet[] = Array(defaultSets).fill(null).map(() => ({
      id: crypto.randomUUID(),
      movementId,
      weight: 0,
      reps: 8,
  ...
  removeExerciseFromWorkout(workoutId: string, exerciseId: string): boolean {
    const workout = this.workouts.find(w => w.id === workoutId);
